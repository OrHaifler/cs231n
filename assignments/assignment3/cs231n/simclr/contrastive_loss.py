...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l_k_N = -torch.log(torch.exp(sim(z_k, z_k_N) / tau)
         / sum([torch.exp(sim(z_k,out[t])/tau) for t in range(2*N) if t != k]))

        l_N_k = -torch.log(torch.exp(sim(z_k_N, z_k) / tau)
         / sum([torch.exp(sim(z_k_N,out[t])/tau) for t in range(2*N) if t != k + N]))
        
        total_loss += l_k_N + l_N_k
...
def sim_positive_pairs(out_left, out_right):
    ...
    norms = out_left.norm(dim = 1) * out_right.norm(dim = 1)
    pos_pairs = torch.sum(out_left * out_right, axis = 1) / norms 
...
def compute_sim_matrix(out):
    ...
    for i in range(out.shape[0]):
      for j in range(out.shape[0]):
        sim_matrix[i,j] /= out[i].norm() * out[j].norm() #Change this!
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    pairs = torch.empty(2*N)
    for k in range(N):
      pairs[k] = sim_matrix[k,k+N]
    pairs[N:] = pairs[:N]
    ...
    for i in range(N):
      loss += -torch.log((numerator[i]/denominator[i]).sum())
    loss /= 2*N
